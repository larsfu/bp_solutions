...
def get_OneTurn(matrizen,N):
    oneTurn=np.eye(6)
    for i in range(N):
            oneTurn=np.matmul(matrizen[:,:,i],oneTurn)
    return oneTurn
...
M,L,S,N=read_lattice('input_files/FODO.opt')
...
C_x=OneTurnMatrix[1,1]
S_x=-OneTurnMatrix[0,1]
C_prime_x=-OneTurnMatrix[1,0]
S_prime_x=OneTurnMatrix[0,0]

C_y=OneTurnMatrix[3,3]
S_y=-OneTurnMatrix[2,3]
C_prime_y=-OneTurnMatrix[3,2]
S_prime_y=OneTurnMatrix[2,2]

beta_x_0=2*S_x/np.sqrt(2-C_x**2-2*S_x*C_prime_x-S_prime_x**2)
alpha_x_0=(C_x-S_prime_x)*beta_x_0/(2*S_x)
gamma_x_0=(1+alpha_x_0**2)/beta_x_0

beta_y_0=2*S_y/np.sqrt(2-C_y**2-2*S_y*C_prime_y-S_prime_y**2)
...
D=D_vec[0,:]
R=np.ones(anzahl_Elemente+1)*1e10
for i in range(anzahl_Elemente):
    if M_Ring[i]==2:
        R[i]=S_Ring[i]
...
#Phasenvorschub
Psi=np.trapz(1/twiss_x[0,:],s)

print('momentum compaction factor: '+str(alpha))
print('Phasenvorschub: '+str(Psi))
...
plt.savefig('build/1_11/beta.png', dpi=200)
...
plt.legend()
plt.tight_layout(pad=0.5)
plt.savefig('build/1_11/alpha.png', dpi=200)
...
plt.legend()
plt.tight_layout(pad=0.5)
plt.savefig('build/1_11/gamma.png', dpi=200)
...
plt.legend()
plt.tight_layout(pad=0.5)
plt.savefig('build/1_11/D.png', dpi=200)
